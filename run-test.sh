...
docker run --rm --user $UID:$UID -v $PWD/test:/opt swagger2postman-cli --file=swagger.json --basepath=asdf
sleep 2

#echo Test multiple environments, some with spaces in name
#docker run --rm --user $UID:$UID -v $PWD/test:/opt swagger2postman-cli --env.test.port=85 --env.test.host=hostname '--env.Test 2.host=Some Value' --file=swagger.json
#sleep 2
